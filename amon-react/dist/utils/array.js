 ...
 */
const mapByKey = (arr, keys) => arr.map(obj => {
    let newObj = {};
 ...
 */
const countFrequencies = (arr, id) => arr.reduce((acc, obj) => acc.set(obj[id], (acc.get(obj[id]) || 0) + 1), new Map());
/**
 ...
 */
const sortMap = (map, srt) => new Map([...map.entries()].sort((a, b) => srt(a, b)));
/**
 ...
 */
const checkSomeFilters = (el, fil) => fil.some(func => func(el));
/**
 ...
 * @returns {boolean}        : whether the element passed or not
 */
const checkEveryFilters = (el, fil) => fil.every(func => func(el));
/**
 ...
 */
const flatComplete = (arr) => arr.reduce((acc, v) => acc.concat(Array.isArray(v) ? flatComplete(v) : v), []);
/**
 ...
 */
const findValueIn = (arr, val) => arr.find(e => val.some(v => e === v));
/**
 ...
 */
const symDifference = (arr1, arr2) => arr1.filter(e => !arr2.includes(e)).concat(arr2.filter(e => !arr1.includes(e)));
/**
 ...
 */
const unique = (arr, keys) => arr.filter((el, i, arr) => arr.findIndex(v => (keys.every(k => v[k] === el[k]))) === i);
export { mapByKey, countFrequencies, sortMap, checkSomeFilters, checkEveryFilters, applySomeFilters, applyEveryFilters, chunkArray, flatComplete, findValueIn, intersection, difference, symDifference, unique };
